...
CREATE TABLE IF NOT EXISTS user_role (
  `user_role_id` BIGINT(22) UNSIGNED NOT NULL AUTO_INCREMENT,
  `username` VARCHAR(64) CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci' NOT NULL,
  `role_name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`user_role_id`),
  UNIQUE INDEX `user_role_id_UNIQUE` (`user_role_id` ASC),
  KEY `usernameind` (`username`)
) ENGINE = INNODB;

CREATE TABLE `radius`.`user_cert` (
  `cert_id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  `common_name` VARCHAR(128) NULL,
  `cert_serial` VARCHAR(256) NULL,
  `dn` VARCHAR(512) NULL,
  `issue_date` DATETIME NULL,
  `expiration_date` DATETIME NULL,
  `is_downloaded` TINYINT(1) NULL,
  `is_revoked` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0,
  `revocation_date` DATETIME NULL,
  PRIMARY KEY (`cert_id`),
  UNIQUE INDEX `cert_id_UNIQUE` (`cert_id` ASC),
  INDEX `index3` (`common_name` ASC),
  INDEX `index4` (`expiration_date` ASC),
  INDEX `index5` (`is_downloaded` ASC),
  INDEX `index6` (`is_revoked` ASC));
