 ...
 * FUNC: openvpn_plugin_func_v1 OPENVPN_PLUGIN_TLS_VERIFY (called once for every
 * cert in the server chain) FUNC: openvpn_plugin_func_v1
 * OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY FUNC: openvpn_plugin_func_v1
 * OPENVPN_PLUGIN_TLS_FINAL FUNC: openvpn_plugin_func_v1 OPENVPN_PLUGIN_IPCHANGE
 *
 * [If OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY returned
 * OPENVPN_PLUGIN_FUNC_DEFERRED, we don't proceed until authentication is
 * verified via auth_control_file]
 *
 ...
 *
 * [If OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY returned
 * OPENVPN_PLUGIN_FUNC_DEFERRED, we expect that authentication is verified via
 * auth_control_file within the number of seconds defined by the "hand-window"
 * option.  Data channel traffic will continue to flow uninterrupted during this
 * period.]
 *
 ...
 * [ some time may pass ]
 *
 * FUNC: openvpn_plugin_func_v1 OPENVPN_PLUGIN_LEARN_ADDRESS (this coincides
 * with a lazy free of initial learned addr object) Server Shutdown:
 *
 ...
 */
typedef enum { SSLAPI_NONE, SSLAPI_OPENSSL, SSLAPI_MBEDTLS } ovpnSSLAPI;
 ...
 * script types which the plug-in wants to intercept.  For example, if the
 ...
 * This pointer is passed on to the other plug-in calls.
 *
 ...
 * processing/usage by the OpenVPN executable.
 *
 */
 ...
 */
OPENVPN_PLUGIN_DEF openvpn_plugin_handle_t OPENVPN_PLUGIN_FUNC(
    openvpn_plugin_open_v2)(unsigned int* type_mask,
                            const char* argv[],
                            const char* envp[],
                            struct openvpn_plugin_string_list** return_list);
 ...
 * file may be generated asynchronously and may be dynamically updated during
 * the client session, however the client will be blocked from sending or
 * receiving VPN tunnel packets until the packet filter file has been generated.
 * OpenVPN will periodically test the packet filter file over the life of the
 * client instance and reload when modified.  OpenVPN will delete the packet
 * filter file when the client instance goes out of scope.
 *
 ...
 * ARGUMENTS
 *
 * version : fixed value, defines the API version of the OpenVPN plug-in API.
 * The plug-in should validate that this value is matching the
 * OPENVPN_PLUGINv3_STRUCTVER value.
 *
...
OPENVPN_PLUGIN_DEF int OPENVPN_PLUGIN_FUNC(
...
OPENVPN_PLUGIN_DEF openvpn_plugin_handle_t
    OPENVPN_PLUGIN_FUNC(openvpn_plugin_open_v1)(unsigned int* type_mask,
                                                const char* argv[],
                                                const char* envp[]);
